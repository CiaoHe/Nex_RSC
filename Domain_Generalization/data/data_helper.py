...
nex_datasets = ['Jul', 'Augu', 'Sep', 'Oct',
                'Nov', 'Dec', 'batch_2', 'batch_3', ]
new_nex_datasets = ['Nex_trainingset', 'Jan2021', 'Feb2021', 'Mar2021']
available_datasets = office_datasets + pacs_datasets + \
    vlcs_datasets + digits_datasets + nex_datasets + new_nex_datasets
...
def get_train_dataloader(args, patches):
    ...
    for dname in dataset_list:
        ...
        name_train, labels_train = _dataset_info(
            join(index_root, "%s_train.txt" % dname))
        name_val, labels_val = _dataset_info(
            join(index_root, "%s_val.txt" % dname))
        ...
        val_datasets.append(JigsawTestNewDataset(data_root, name_val, labels_val,
                                                 img_transformer=get_val_transformer(args), patches=patches, jig_classes=30))
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True, drop_last=False)
...
def get_val_dataloader(args, patches=False):
    ...
    val_dataset = JigsawTestNewDataset(
        data_root, names, labels, patches=patches, img_transformer=img_tr, jig_classes=30)
    ...
    loader = torch.utils.data.DataLoader(
        dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=False)
    return loader
...
def get_tgt_dataloader(args, patches=False):
    ...
    if args.downsample_target:
        name_train, labels_train = _dataset_info(
            join(index_root, "%s_train_down.txt" % dname))
        name_val, labels_val = _dataset_info(
            join(index_root, "%s_val_down.txt" % dname))
        name_test, labels_test = _dataset_info(
            join(index_root, "%s_test_down.txt" % dname))
    else:
        name_train, labels_train = _dataset_info(
            join(index_root, "%s_train.txt" % dname))
        name_val, labels_val = _dataset_info(
            join(index_root, "%s_val.txt" % dname))
        name_test, labels_test = _dataset_info(
            join(index_root, "%s_test.txt" % dname))

    tgt_train_dataset = JigsawTestNewDataset(
        data_root, name_train, labels_train, patches=patches, img_transformer=img_tr, jig_classes=30)
    ...
    tgt_dataset = ConcatDataset(
        [tgt_train_dataset, tgt_val_dataset, tgt_test_dataset])
    loader = torch.utils.data.DataLoader(
        tgt_dataset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True, drop_last=False)
    return loader


def get_train_transformers(args):
    img_tr = [transforms.RandomResizedCrop(
        (int(args.image_size), int(args.image_size)), (args.min_scale, args.max_scale))]
    if args.random_horiz_flip > 0.0:
        img_tr.append(transforms.RandomHorizontalFlip(args.random_horiz_flip))
    if args.jitter > 0.0:
        img_tr.append(transforms.ColorJitter(brightness=args.jitter,
                                             contrast=args.jitter, saturation=args.jitter, hue=min(0.5, args.jitter)))
    ...
    tile_tr = tile_tr + [transforms.ToTensor(), transforms.Normalize(
