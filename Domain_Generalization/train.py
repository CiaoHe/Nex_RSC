...
def get_args():
    ...
    # hardware
    parser.add_argument("--cuda_number", choices=[i for i in range(
        torch.cuda.device_count())], help="Choose use which cuda", default=0, type=int)
    # dataset to use
    parser.add_argument("--source", type=str, default='',
                        help="Source, use '-' to connect")
    ...
    # data aug stuff
    parser.add_argument("--min_scale", default=0.8,
                        type=float, help="Minimum scale percent")
    parser.add_argument("--max_scale", default=1.0,
                        type=float, help="Maximum scale percent")
    parser.add_argument("--random_horiz_flip", default=0.5,
                        type=float, help="Chance of random horizontal flip")
    parser.add_argument("--jitter", default=0.4, type=float,
                        help="Color jitter amount")
    parser.add_argument("--tile_random_grayscale", default=0.1,
                        type=float, help="Chance of randomly greyscaling a tile")
    # criterion
    parser.add_argument("--loss", default='ce', type=str, help="loss function")
    parser.add_argument("--save_metric", default="AUC",
                        type=str, help="Metric of saving best model (AUC/FPR)")
    #
    parser.add_argument("--limit_source", default=None, type=int,
                        help="If set, it will limit the number of training samples")
    parser.add_argument("--limit_target", default=None, type=int,
                        help="If set, it will limit the number of testing samples")
    parser.add_argument("--downsample_target", action='store_true',
                        help="Use downsampled target to evaluate")
    ...
    parser.add_argument("--n_classes", "-c", type=int,
                        default=7, help="Number of classes")
    parser.add_argument("--network", choices=model_factory.nets_map.keys(),
                        help="Which network to use", default="resnet18")
    ...
    parser.add_argument("--folder_name", default='test',
                        help="Used by the logger to save logs")
    ...
    parser.add_argument("--nesterov", default=False,
                        type=bool, help="Use nesterov")
    # misc
    parser.add_argument("--eval_mode", action='store_true',
                        help="Launch evaluation mode(default:False)")
    parser.add_argument("--save_model", action='store_true',
                        help="Save model's params")
    parser.add_argument("--RSC_flag", action='store_true',
                        help="Whether use RSC, default True")
    parser.add_argument("--pretrained", action='store_true',
                        help="Whether use Pretrained model, default True")
    parser.add_argument("--resume", default='', type=str, metavar='PATH',
                        help="path to latest checkpoint(default:none)")
    parser.add_argument("--infer_model", default='', type=str,
                        metavar='PATH', help="Inference model's path(default:none)")
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.optimizer, self.scheduler = get_optim_and_scheduler(
            model, args.epochs, args.learning_rate, args.train_all, nesterov=args.nesterov)
    ...
    def do_test(self, loader):
        ...
        auc_dict = {'auc': auc, 'fpr_980': fpr_980, 'fpr_991': fpr_991, 'fpr_993': fpr_993,
                    'fpr_995': fpr_995, 'fpr_997': fpr_997, 'fpr_999': fpr_999, 'fpr_1': fpr_1, 'thresholds': thresholds}
        return class_correct, auc_dict

    def do_training(self):
        self.logger = Logger(self.args, update_frequency=50)
        self.results = {"val": torch.zeros(
            self.args.epochs), "test": torch.zeros(self.args.epochs)}
    ...
    def save_model(self, epoch, auc_dict):
                ...
                if tmp_auc > rec:
                    for j in range(len(self.topk)-1, i, -1):
                        ...
                        _j, _jm1 = join(_save_models_dir, f"tgt_{self.args.target}_src_{'-'.join(self.args.source)}_RSC_{self.args.RSC_flag}_best{j+1}.pth"),\
                            join(
                                _save_models_dir, f"tgt_{self.args.target}_src_{'-'.join(self.args.source)}_RSC_{self.args.RSC_flag}_best{j}.pth")
                        if exists(_jm1):
                            os.rename(_jm1, _j)
                    self.topk[i] = tmp_auc
                    model_saved_path = join(
                        _save_models_dir, f"tgt_{self.args.target}_src_{'-'.join(self.args.source)}_RSC_{self.args.RSC_flag}_best{i+1}_{self.args.save_metric}.pth")
                    state_to_save = {'model': self.model.state_dict(
                    ), 'auc_dict': auc_dict, 'epoch': epoch}
        ...
        if epoch in range(self.args.epochs-3, self.args.epochs):
            model_saved_path = join(
                _save_models_dir, f"tgt_{self.args.target}_src_{'-'.join(self.args.source)}_RSC_{self.args.RSC_flag}_last{self.args.epochs-epoch}_{self.args.save_metric}.pth")
            state_to_save = {'model': self.model.state_dict(
            ), 'auc_dict': auc_dict, 'epoch': epoch}
            torch.save(state_to_save, model_saved_path)
            print(
